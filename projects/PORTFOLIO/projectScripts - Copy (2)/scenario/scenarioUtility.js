    ...
    return new Promise((resolve) => {
        ...
        new TWEEN.Tween(tObj)
            ...
            .onComplete(() => {
                ...
                drone.lookAt(scene.camera.position);
                quat.copy(drone.quaternion);
                drone.rapierBody.setRotation(quat, true);
                            ...
                            if (ball.rapierCollider) {
                                try {
                                    if (typeof ball.rapierCollider.setRadius === 'function') {
                                        // Geometry Radius is 0.5, so VisualRadius = 0.5 * Scale
                                        // We want ColliderRadius = VisualRadius (1:1 ratio as per original)
                                        // So NewRadius = 0.5 * Scale
                                        ball.rapierCollider.setRadius(ball.scale.x * 0.5);
                                    }
                                } catch (e) {
                                    // Suppress errors during animation to avoid spam
                                }
                            }
                        })
                        .start();
