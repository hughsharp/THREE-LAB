export const vertexShader = `
    ...
    void main() {
        ...
        float cycle = mod(iTime + rndTex * 10.0, 3.5);
        float isBuzzPhase = step(3.0, cycle); 

        float isActive = isHover * isBuzzPhase;
        
        // Use floor(time) to snap through frames
        float steppedTime = floor((iTime * baseSpeed) + rndTex) * isActive;
        
        // If not hovering (isHover=0), steppedTime is 0, so index stays static (rndTex*32)
