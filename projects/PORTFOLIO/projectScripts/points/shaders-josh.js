export const vertexShader = `
    ...
    uniform float uModelRotationX; // New uniform for X rotation
    uniform float uModelRotationY; // New uniform for rotation
    uniform float uModelRotationZ; // New uniform for Z rotation
    uniform float uModelScale;      // New uniform for scale
    uniform vec3 uModelPosition;    // New uniform for XYZ offset
    ...
    void main() {
        ...
        // Texture index logic moved to end of main() to use 'dist'
        ...
        // --- SKINNING & TARGET CALCULATION ---
        vec3 finalTarget = aTargetPos;

        // Apply skinning ONLY if it's a model point (isGrid < 0.5)
        // Note: We use a step function or if-check.
        // But for shaders, if-checks are fine if coherent.
        
        bool isModelPoint = (aIsGrid < 0.5);

        if (isModelPoint) {
           vec3 transformed = aTargetPos; // Skinning source is our target Model position
           #include <skinbase_vertex>
           #include <skinning_vertex>
           finalTarget = transformed;
           
           // Apply Transformations to Model Target Only
           finalTarget = rotateX(uModelRotationX) * rotateY(uModelRotationY) * rotateZ(uModelRotationZ) * finalTarget;
           finalTarget *= uModelScale;
           finalTarget += uModelPosition;
        }

        // Morph from position (random) to target position (finalTarget) based on uProgress
        vec3 morphedPos = mix(position, finalTarget, uProgress);
        ...
        float isModelFloat = 1.0 - step(0.001, aIsGrid);
        gl_Position.xy += uModelOffset * gl_Position.w * isModelFloat;
        ...
        // Check mouse distance (calculated above as 'dist')
        // If within hover radius but OUTSIDE the direct interaction zone (minInteractionDist), enable animation
        // This keeps the point directly under the mouse static (no flicker).
        // minInteractionDist is defined above in Repulsion logic
        ...
        float steppedTime = floor((iTime * animationSpeed) + rndTex) * isHover; // Only advance time if in the "donut" zone
        
        vTextureIndex = floor(mod((rndTex * 32.0) + steppedTime, 32.0));
