...
export function createUI({
    ...
    (() => {
        ...
        const initial = (material && material.uniforms && material.uniforms.uHoverLightFactor) ? material.uniforms.uHoverLightFactor.value : 1.0;

        const text = document.createElement('span');
        text.innerText = `Hover Light: ${initial.toFixed(2)}`;
        text.style.fontSize = '12px';
        ...
        const updateUI = () => {
            if (material && material.uniforms && material.uniforms.uHoverLightFactor) {
                const v = material.uniforms.uHoverLightFactor.value;
                text.innerText = `Hover Light: ${v.toFixed(2)}`;
                number.value = v;
                input.value = v;
            }
        };
        ...
        function setValue(v) {
            const val = parseFloat(v) || 0;
            text.innerText = `Hover Light: ${val.toFixed(2)}`;
            number.value = val;
            input.value = val;
            highlightIfChanged(number, val, 1.0);
            if (material && material.uniforms && material.uniforms.uHoverLightFactor) {
                material.uniforms.uHoverLightFactor.value = val;
            }
        }
