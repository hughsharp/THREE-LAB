...
export function bindBodyObject(scene, object, body, shape, options = {}) {
    ...
    object.rapierBody.setTranslation(object.position)
    object.rapierBody.setRotation(object.quaternion)
    scene.world.createCollider(object.rapierShape, object.rapierBody);
}
...
export function getFreeFormBodyShapeFromMesh(scene, mesh, options = {}) {
    ...
    const sleepThreshold = options.sleepThreshold ?? 0.5; // New Default 0.5
    const linearDamping = options.linearDamping ?? 0.1;   // Default 0.1
    const angularDamping = options.angularDamping ?? 0.1; // Default 0.1
    ...
    // Apply sleep threshold if valid
    if (desc.setSleepThreshold) desc.setSleepThreshold(sleepThreshold);
