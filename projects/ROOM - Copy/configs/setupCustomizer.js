...
export class Customizer {
    ...
    _populateCustomizePanel() {
        ...
        const posRow = this._createEditRow('Position:', obj.position, () => this._updateInfoPanel(), originalState.position);
        body.appendChild(posRow);

        const rotRow = this._createEditRow('Rotation:', obj.rotation, () => this._updateInfoPanel(), originalState.rotation);
        ...
        body.appendChild(this._createEditRow('Scale:', obj.scale, () => this._updateInfoPanel(), originalState.scale));
    ...
    _generateMaterialUI(mat, container) {
        ...
        let envMapRotationRow = null;
        if ('envMapRotation' in mat) {
            envMapRotationRow = this._createEditRow('EnvMap Rotation (rad):', mat.envMapRotation, () => {
                mat.needsUpdate = true;
            }, originalState.envMapRotation);
            envMapRotationRow.style.marginTop = '8px';
        }
        ...
        const updateEnvMapVisibility = () => {
            ...
            if (envMapRotationRow) envMapRotationRow.style.display = isChecked ? 'flex' : 'none';
        };

        if (envMapIntensityRow) envMapIntensityRow.style.display = envMapCheckboxData.checkbox.checked ? 'flex' : 'none';
        if (envMapRotationRow) envMapRotationRow.style.display = envMapCheckboxData.checkbox.checked ? 'flex' : 'none';
        ...
        if (envMapRotationRow) container.appendChild(envMapRotationRow);
    ...
    _createEditRow(label, vector, onChange, originalVector = null) {
        const row = document.createElement('div');
        row.style.display = 'flex';
        row.style.alignItems = 'center';
        row.style.gap = '8px';
    ...
    _createMaterialControl(label, type, value, onChange, options = {}, originalValue = undefined) {
        ...
        if (type === 'color') {
            ...
            valueInput.step = options.step ?? 0.01;
            ...
            maxInput.step = options.step ?? 0.01;
