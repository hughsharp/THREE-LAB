...
function addBulb(scene) {
    ...
    bulbMat.uniforms.iTime = constantUniform.iTime;
    bulbMat.uniforms.uOscillationStrength = constantUniform.uOscillationStrength
    bulbMat.uniforms.uIsOscillating = constantUniform.uIsOscillating
    ...
    let bulbAura = new THREE.Mesh(bulb.geometry, CONSTANTS.goldOscillatingOuterGlowMat);
    // let bulbAura = bulb.clone()
    bulbAura.material = CONSTANTS.goldOscillatingOuterGlowMat;
    bulbAura.material.uniforms.iTime = constantUniform.iTime; // Link time
    bulbAura.material.uniforms.uOscillationStrength = constantUniform.uOscillationStrength
    bulbAura.material.uniforms.uIsOscillating = constantUniform.uIsOscillating
    bulbAura.scale.setScalar(2.5);
...
function bindRapierToModelObjects(scene, progressText) {
    ...
    return new Promise((resolve, reject) => {
        ...
        specifyBinding("Object_2001", ARAP.getFreeFormBodyShapeFromMesh, { bodyType: 'dynamic', mass: 80, restitution: 0.3, canSleep: true, isBhTarget: true, yOffset: -0.1, pullingDampness: 0.45, sleepThreshold: 2.0, linearDamping: 0.5, angularDamping: 0.5 }) // CHAIR
        specifyBinding("mjolnir_low_mjolnir_hammer_0", ARAP.getFreeFormBodyShapeFromMesh, { bodyType: 'dynamic', mass: 10, restitution: 0.0, canSleep: true, isBhTarget: true, pullingDampness: 0.9075, isConvexHull: true, sleepThreshold: 2.0, linearDamping: 0.5, angularDamping: 0.5 }) // MJOLNIR
