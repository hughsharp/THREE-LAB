...
export default class MeshReflectorMaterial extends MeshStandardMaterial {
    // --- NEW: Map of property names to their actual min/max ranges for UI mapping ---
    _propertyRanges = {
        mirror: { min: 0, max: 1 },
        mixStrength: { min: 0, max: 5 },
        mixBlur: { min: 0, max: 1 },
        mixContrast: { min: 0, max: 2 },
        distortion: { min: 0, max: 5 },
        minDepthThreshold: { min: 0, max: 1 },
        maxDepthThreshold: { min: 0, max: 1 },
        depthScale: { min: 0, max: 50 },
        depthToBlurRatioBias: { min: 0, max: 1 },
    };
        ...
        shader.fragmentShader = shader.fragmentShader.replace(
            ...
            float blurFactor = 0.0;
                ...
                depthFactor = max(0.0001, min(1.0, depthFactor));
                    ...
                    // Corrected Line
                    // blur = blur * min(1.0, (1.0 - depthFactor) + depthToBlurRatioBias);
            ...
            diffuseColor.rgb = mix(diffuseColor.rgb, newMerge.rgb * mixStrength, mirror);
            `
        );
    }
    ...
    _sliderToActual(sliderValue, propName) {
        const range = this._propertyRanges[propName];
        if (!range) return sliderValue;
        return range.min + (sliderValue / 100) * (range.max - range.min);
    }

    _actualToSlider(actualValue, propName) {
        const range = this._propertyRanges[propName];
        if (!range) return actualValue;
        const value = ((actualValue - range.min) / (range.max - range.min)) * 100;
    ...
    _createHelperUI() {
        ...
        body.appendChild(this._createSliderControl('mirror'));
        body.appendChild(this._createSliderControl('mixStrength'));
        body.appendChild(this._createSliderControl('mixBlur'));
        body.appendChild(this._createSliderControl('mixContrast'));
        body.appendChild(this._createSliderControl('distortion'));
        body.appendChild(this._createNumberControl('reflectorOffset', { step: 0.01 }));
        body.appendChild(this._createSliderControl('minDepthThreshold'));
        body.appendChild(this._createSliderControl('maxDepthThreshold'));
        body.appendChild(this._createSliderControl('depthScale'));
        body.appendChild(this._createSliderControl('depthToBlurRatioBias'));

        // Special controls for arrays and vectors
        body.appendChild(this._createArrayControl('blur', 2, { min: 0, max: 1024, step: 1 }));
        body.appendChild(this._createVectorControl('planeNormal'));
    ...
    _createSliderControl(propName) {
        const row = document.createElement('div');
        row.style.display = 'grid';
        row.style.gridTemplateColumns = '110px 1fr 45px';
        ...
        const initialActualValue = this.reflectorProps[propName];
        slider.value = this._actualToSlider(initialActualValue, propName);
        numberInput.value = initialActualValue.toFixed(2);

        slider.oninput = () => {
            const actualValue = this._sliderToActual(slider.value, propName);
            this.reflectorProps[propName] = actualValue;
            numberInput.value = actualValue.toFixed(2);
        };

        numberInput.onchange = () => {
            let actualValue = parseFloat(numberInput.value) || 0;
            const range = this._propertyRanges[propName];

            if (range) {
                actualValue = Math.max(range.min, Math.min(range.max, actualValue));
            }

            this.reflectorProps[propName] = actualValue;
            slider.value = this._actualToSlider(actualValue, propName);
            numberInput.value = actualValue.toFixed(2);
        };
    ...
    _createNumberControl(propName, options = {}) {
        const row = document.createElement('div');
        row.style.display = 'grid';
        ...
        input.oninput = () => {
            this[propName] = parseFloat(input.value) || 0;
        };
    ...
    _createArrayControl(propName, size, options = {}) {
        const row = document.createElement('div');
        row.style.display = 'grid';
        row.style.gridTemplateColumns = '110px 1fr';
        row.style.alignItems = 'center';
        ...
        for (let i = 0; i < size; i++) {
            ...
            input.type = 'number';
            Object.assign(input, options);
            input.value = this[propName][i];
            input.style.width = '50px';
            input.onchange = () => {
                this[propName][i] = parseFloat(input.value) || 0;
                this.hasBlur = this.blur[0] + this.blur[1] > 0;
                this.reflectorProps.hasBlur = this.hasBlur;
                this.reflectorProps['defines-USE_BLUR'] = this.hasBlur ? '' : undefined;
                this.kawaseBlurPass.setSize(this.blur[0], this.blur[1]);
    ...
    _createVectorControl(propName) {
        ...
        ['x', 'y', 'z'].forEach(axis => {
            ...
            input.type = 'number';
            input.step = 0.01;
            input.value = this[propName][axis].toFixed(2);
            input.style.width = '50px';
            input.onchange = () => {
                this[propName][axis] = parseFloat(input.value) || 0;
            };
            inputBox.appendChild(input);
        });
        row.appendChild(inputBox);
